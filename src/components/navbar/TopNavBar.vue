<template>
    <div class="flex items-center justify-between gap-5">
        ...
        <div class="flex gap-5 items-center justify-center 2xs:hidden xs:hidden sm:hidden md:hidden lg:flex">
            <div v-for="(item, index) in navBarItems" @click="$router.push(item.path as string)" class="cursor-pointer">
    ...
    <div v-if="showMenu" class="fixed bg-white w-2/3 p-3 shadow-md 2xs:block md:block lg:hidden 2xl:hidden z-[10000]">
        <menu v-for="(item, index) in navBarItems" @click="$router.push(item.path as string)" class="cursor-pointer">
            <p class="p-2 whitespace-nowrap text-sm"
                :class="item.path === $route.path ? 'bg-[#FD6F00] text-[#fff] rounded-md' : ''"> {{ item.name }}</p>
        </menu>
    </div>
